  ...
  <div style="background: white; padding: 10px">
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.title === '操作'">
          <a @click="EditHandler(record)">
            <EditTwoTone />
            ️
          </a>
          <a-divider type="vertical" />
          <a @click="DeleteLogHandler(record.id)"><DeleteTwoTone /></a>
        </template>
    ...
    <a-modal v-model:open="data.showEditModal" title="修改日志" @ok="handleOk">
      ...
      >
        ...
        >
          <a-textarea :rows="5" v-model:value="logItem.content"></a-textarea>
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
